...
export function calculateExpenseSummary(
  ...
  const { start, end, formatInterval, getIntervals } = getDateRangeConfig(
    dateRange,
    customStart,
    customEnd
  );
  ...
  return {
    ...
    byCategory: filteredExpenses.reduce(
      (acc, exp) => {
...
function getCategoryData(expenses: Expense[], categories: Category[]) {
  ...
  return {
    labels: sortedCategories.map(([id]) => categories.find(c => c.id === id)?.name || id),
